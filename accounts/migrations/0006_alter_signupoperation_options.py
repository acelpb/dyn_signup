...
def convert_from_signup_to_bill(apps, schema_editor):
    ...
    OperationValidation.objects.filter(
        ...
        bill_id=Subquery(Signup.objects.filter(id=OuterRef('object_id')).values('bill__id')[:1])
    ).update(
...
def convert_from_bill_to_signup(apps, schema_editor):
    ...
    OperationValidation.objects.filter(
        ...
        signup_id=Subquery(Signup.objects.filter(bill__id=OuterRef('object_id')).values('id')[:1])
    ).update(
        content_type=ContentType.objects.get_by_natural_key("signup2022", "signup"),
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.AlterModelOptions(
            ...
            options={'verbose_name': 'paiement inscriptions 2022',
